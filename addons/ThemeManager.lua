...
if typeof(clonefunction) == "function" then
    -- 为某些漏洞利用修复is_____函数，这些函数不应该出错，只应返回布尔值
    local
        isfolder_copy,
        isfile_copy,
...
do
    ...
    function ThemeManager:SetDefaultTheme(theme)
        ...
        assert(not self.AppliedToTab, "在将ThemeManager应用到标签页后无法设置默认主题！")
    ...
    function ThemeManager:SaveCustomTheme(file)
        if file:gsub(" ", "") == "" then
            self.Library:Notify("主题文件名无效（空名称）", 3)
            return
        end
    ...
    Values = {
        ...
        groupbox:AddButton("创建主题", function()
            ...
            if name:gsub(" ", "") == "" then
                self.Library:Notify("主题名称无效（空名称）", 2)
                return
            end
        ...
        groupbox:AddButton("重置默认", function()
            ...
            if not success then
                self.Library:Notify("重置默认失败: 删除文件时出错")
                return
            end

            self.Library:Notify("已将默认主题设置为空")
            self.Library.Options.ThemeManager_CustomThemeList:SetValues(self:ReloadCustomThemes())
            self.Library.Options.ThemeManager_CustomThemeList:SetValue(nil)
        end)
