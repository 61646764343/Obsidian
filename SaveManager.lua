...
if typeof(clonefunction) == "function" then
    -- 修复某些漏洞利用中的is_____函数，这些函数不应出错，只应返回布尔值
...
local SaveManager = {} do
    ...
    function SaveManager:IgnoreThemeSettings()
        self:SetIgnoreIndexes({
            "BackgroundColor", "MainColor", "AccentColor", "OutlineColor", "FontColor", "FontFace", -- 主题相关
            "ThemeManager_ThemeList", "ThemeManager_CustomThemeList", "ThemeManager_CustomThemeName", -- 主题相关
        })
    end
    ...
    function SaveManager:Load(name)
        ...
        for _, option in pairs(decoded.objects) do
            ...
            task.spawn(self.Parser[option.type].Load, option.idx, option) -- 使用task.spawn()避免配置加载被卡住
        end
    ...
    function SaveManager:RefreshConfigList()
        ...
        if (not success) then
            if self.Library then
                self.Library:Notify("加载配置列表失败: " .. tostring(data))
            else
                warn("加载配置列表失败: " .. tostring(data))
            end
    ...
    function SaveManager:LoadAutoloadConfig()
        ...
        if isfile(autoLoadPath) then
            local successRead, name = pcall(readfile, autoLoadPath)
            if not successRead then
                self.Library:Notify("加载自动加载配置失败: 写入文件错误")
                return
            end

            local success, err = self:Load(name)
            if not success then
                self.Library:Notify("加载自动加载配置失败: " .. err)
                return
            end
    ...
    function SaveManager:BuildConfigSection(tab)
        ...
        section:AddButton("创建配置", function()
            ...
            if name:gsub(" ", "") == "" then
                self.Library:Notify("无效的配置名称（空）", 2)
                return
            end
        ...
        section:AddButton("设为自动加载", function()
            ...
            if not success then
                self.Library:Notify("设置自动加载配置失败: " .. err)
                return
            end

            self.Library:Notify(string.format("已将 %q 设置为自动加载", name))
        ...
        section:AddButton("重置自动加载", function()
            ...
            self.Library:Notify("已将自动加载设置为无")
