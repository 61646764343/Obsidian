...
do
    ...
    function CustomImageManager.AddAsset(
        ...
        assert(typeof(RobloxAssetId) == "number", "Roblox资源ID必须是数字")
...
local function CheckDepbox(Box, Search)
    ...
    for _, ElementInfo in Box.Elements do
        ...
        elseif ElementInfo.SubButton then
            ...
            if ElementInfo.Text:lower():match(搜索) and ElementInfo.Visible then
                Visible = true
            else
                ElementInfo.Base.Visible = false
            end
            if ElementInfo.SubButton.Text:lower():match(搜索) and ElementInfo.SubButton.Visible then
                Visible = true
            else
...
function Library:ChangeCursorIconSize(Size: UDim2)
    assert(typeof(Size) == "UDim2", "期望UDim2类型。")
...
function Library:CreateWindow(WindowInfo)
    ...
    function Window:ChangeTitle(title)
        assert(typeof(title) == "string", "期望标题为字符串，得到：" .. typeof(title))
