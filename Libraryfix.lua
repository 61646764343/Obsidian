...
local Config = {
    ...
    SafeMode = false, -- 如果为true，禁用外部加载
    DefaultTimeout = 10, -- 网络请求超时（秒）
    IconCacheTime = 86400, -- 图标缓存时间（秒）
}
...
-- 添加性能检查函数
local function PerformanceCheck(funcName, func, ...)
    if not Config.EnablePerformanceLogging then
        return func(...)
    end
    ...
    if duration > 0.01 then -- 超过10ms警告
        warn(string.format("[Obsidian性能警告] %s 耗时 %.3f秒", funcName, duration))
    end
    
    if DebugTools.Enabled then
        table.insert(DebugTools.PerformanceLogs, {
            Function = funcName,
            Duration = duration,
            Timestamp = tick()
        })
        ...
        -- 保持最多100条记录
        if #DebugTools.PerformanceLogs > 100 then
            table.remove(DebugTools.PerformanceLogs, 1)
        end
    end
...
-- 添加类型验证函数
local function ValidateType(value, expectedType, fieldName, allowNil)
    if allowNil and value == nil then
        return true
    end
    
    if typeof(value) ~= expectedType then
        error(string.format("[Obsidian类型错误] %s 应为 %s 类型，实际为 %s", 
              fieldName, expectedType, typeof(value)))
        return false
    end
...
-- 安全哈希验证函数（简单实现）
local function ValidateContentHash(content, expectedHash)
    if not expectedHash then
        return true
    end
    
    -- 简单的哈希计算（实际应用中应该使用更安全的哈希算法）
    local hash = 0
    for i = 1, #content do
        hash = (hash * 31 + string.byte(content, i)) % 2^32
    end
    
    return tostring(hash) == expectedHash
end

-- 检查文件类型
local function CheckFileType(content, allowedTypes)
    if not content or #content == 0 then
        return false, "内容为空"
    end
    
    -- PNG文件头
    if content:sub(1, 8):byte() == 0x89 and content:sub(2, 4) == "PNG" then
        return true, "png"
    end
    
    -- JPEG文件头
    if content:sub(1, 2):byte() == 0xFF and content:sub(2, 3):byte() == 0xD8 then
        return true, "jpeg"
    end
    
    -- GIF文件头
    if content:sub(1, 3) == "GIF" then
        return true, "gif"
    end
...
do
    ...
    function CustomImageManager.AddAsset(
        ...
        if CustomImageManagerAssets[AssetName] ~= nil then
            error(string.format("资源 %q 已存在", AssetName))
        end

        ValidateType(RobloxAssetId, "number", "Roblox资源ID")
        ...
        return CustomImageManager.DownloadAsset(AssetName, ForceRedownload)
    end
    ...
    function CustomImageManager.DownloadAsset(AssetName: string, ForceRedownload: boolean?)
        ...
        local success, errorMessage = PerformanceCheck("DownloadAsset", function()
            if Config.SafeMode then
                error("安全模式已启用，禁止下载外部资源")
            end
            ...
            local downloadSuccess, downloadResult = pcall(function()
                content = game:HttpGet(AssetData.URL, true)
            end)
            
            if not downloadSuccess then
                error("下载失败: " .. tostring(downloadResult))
            end
            
            if tick() - startTime > Config.DefaultTimeout then
                error("下载超时")
            end
            ...
            if #content > Config.MaxFileSize then
                error(string.format("文件过大 (%d bytes, 限制: %d bytes)", 
                      #content, Config.MaxFileSize))
            end
            
            -- 验证哈希值
            if AssetData.ExpectedHash and not ValidateContentHash(content, AssetData.ExpectedHash) then
                error("文件哈希验证失败")
            end
            
            -- 检查文件类型
            local isValid, fileType = CheckFileType(content, {"png", "jpeg", "gif"})
            if not isValid then
                error("不支持的图片格式: " .. fileType)
            end
        ...
        return success, success and "下载成功" or ("下载失败: " .. tostring(errorMessage))
    end

    for AssetName, _ in CustomImageManagerAssets do
        local success, message = CustomImageManager.DownloadAsset(AssetName)
        if not success and Config.EnablePerformanceLogging then
            warn("[Obsidian] 资源下载失败: " .. AssetName .. " - " .. tostring(message))
        end
    end
end
...
-- 添加调试功能
function Library:EnableDebug(enable: boolean)
    self.DebugTools.Enabled = enable
    self.Config.EnablePerformanceLogging = enable
    
    if enable then
        print("[Obsidian调试] 调试模式已启用")
    end
end
...
function Library:GetElementCount()
    return #self.Labels + #self.Buttons + #self.Toggles + #self.Options
end

function Library:TakeMemorySnapshot(name)
    local snapshot = {
        Name = name or "Snapshot_" .. tick(),
        Time = tick(),
        Memory = self:GetMemoryUsage(),
        Elements = self:GetElementCount(),
        Instances = #self.MemoryTrack.Instances,
        Connections = #self.MemoryTrack.Connections
    }
    
    table.insert(self.DebugTools.MemorySnapshots, snapshot)
    
    -- 保持最多50个快照
    if #self.DebugTools.MemorySnapshots > 50 then
        table.remove(self.DebugTools.MemorySnapshots, 1)
    end
    
    return snapshot
end

function Library:CleanupUnusedMemory()
    local now = tick()
    if now - self.MemoryTrack.LastCleanup < 60 then -- 每分钟最多清理一次
        return
    end
    
    self.MemoryTrack.LastCleanup = now
    
    -- 清理无效的实例引用
    local toRemove = {}
    for instance, _ in pairs(self.MemoryTrack.Instances) do
    ...
    for _, instance in ipairs(toRemove) do
        self.MemoryTrack.Instances[instance] = nil
    end
    
    -- 清理无效的连接
    toRemove = {}
    for i, connection in ipairs(self.MemoryTrack.Connections) do
    ...
    for i = #toRemove, 1, -1 do
        table.remove(self.MemoryTrack.Connections, toRemove[i])
    end
    
    -- 建议垃圾回收
    collectgarbage("step")
    
    if self.DebugTools.Enabled then
        print(string.format("[Obsidian内存清理] 清理了 %d 个实例和 %d 个连接", 
              #toRemove, #self.MemoryTrack.Connections))
    end
end
...
local function GetSchemeValue(Index)
    ...
    if AliasIndex and Library.Scheme[AliasIndex] ~= nil then
        warn(string.format("方案值 %q 已弃用，请使用 %q 代替。", Index, AliasIndex))
...
local function CheckDepbox(Box, Search)
    ...
    for _, ElementInfo in Box.Elements do
        ...
        --// Check if 搜索 matches Element's Name and if Element is Visible
...
function Library:AddToRegistry(Instance, Properties)
    ...
    Library.MemoryTrack.Instances[Instance] = true
end

function Library:RemoveFromRegistry(Instance)
    Library.Registry[Instance] = nil
    Library.MemoryTrack.Instances[Instance] = nil
end
...
function Library:SetDPIScale(DPIScale: number)
    ValidateType(DPIScale, "number", "DPIScale")
...
function Library:GiveSignal(Connection: RBXScriptConnection | RBXScriptSignal)
    ...
    if Connection and (ConnectionType == "RBXScriptConnection" or ConnectionType == "RBXScriptSignal") then
        ...
        table.insert(Library.MemoryTrack.Connections, Connection)
    end
...
local FetchIcons, Icons = pcall(function()
    ...
    local success, result = pcall(function()
        iconContent = game:HttpGet("https://raw.githubusercontent.com/deividcomsono/lucide-roblox-direct/refs/heads/main/source.lua", true)
    end)
    
    if not success then
        error("图标模块下载失败: " .. tostring(result))
    end
    ...
    if not iconContent or #iconContent == 0 then
        error("图标模块内容为空")
    end
    
    -- 检查文件大小
    if #iconContent > Config.MaxFileSize then
        error(string.format("图标模块过大 (%d bytes, 限制: %d bytes)", 
              #iconContent, Config.MaxFileSize))
    end
    
    -- 加载模块
    local moduleFunction = loadstring(iconContent)
...
local LocalIcons = {
    ...
    move = {
        Url = "rbxassetid://3570695787",
        ImageRectOffset = Vector2.new(500, 500),
        ImageRectSize = Vector2.new(50, 50),
    },
    search = {
        Url = "rbxassetid://3570695787",
        ImageRectOffset = Vector2.new(600, 600),
        ImageRectSize = Vector2.new(50, 50),
    }
}
...
local function New(ClassName: string, Properties: { [string]: any }): any
    ValidateType(ClassName, "string", "ClassName")
...
function Library:ChangeCursorIcon(ImageId: string)
    ...
    assert(Icon, "图片必须是有效的Roblox资源、有效URL或lucide图标。")
...
function Library:GetTextBounds(Text: string, Font: Font, Size: number, Width: number?): (number, number)
    ...
    ValidateType(Font, "userdata", "Font")
    ValidateType(Size, "number", "Size")
...
function Library:MakeDraggable(UI: GuiObject, DragFrame: GuiObject, IgnoreToggled: boolean?, IsMainWindow: boolean?)
    ValidateType(UI, "Instance", "UI")
    ValidateType(DragFrame, "Instance", "DragFrame")
    ...
    local connection = DragFrame.InputBegan:Connect(function(Input: InputObject)
    ...
    Library:GiveSignal(moveConnection)
end
...
function Library:MakeLine(Frame: GuiObject, Info)
    ValidateType(Frame, "Instance", "Frame")
...
function Library:MakeOutline(Frame: GuiObject, Corner: number?, ZIndex: number?)
    warn("Obsidian:MakeOutline已弃用，请使用Obsidian:AddOutline代替。")
...
function Library:AddDraggableLabel(Text: string)
    ...
    function Table:SetVisible(Visible: boolean)
        ValidateType(Visible, "boolean", "Visible")
...
function Library:AddDraggableButton(Text: string, Func, ExcludeScaling: boolean?)
    ValidateType(Text, "string", "Text")
    ValidateType(Func, "function", "Func")
...
do
    ...
    function Library:SetWatermark(Text: string)
        warn("Watermark已弃用，请使用Library:AddDraggableLabel代替。")
...
function Library:AddContextMenu(
    ...
    ValidateType(Holder, "Instance", "Holder")
    ...
    function Table:Open()
        ...
        Library:GiveSignal(Table.Signal)
    end
...
function Library:AddTooltip(InfoStr: string, DisabledInfoStr: string, HoverInstance: GuiObject)
    ValidateType(HoverInstance, "Instance", "HoverInstance")
    ...
    function TooltipTable:Destroy()
        for Index = #TooltipTable.Signals, 1, -1 do
            ...
            if Connection and Connection.Connected then
                ...
                -- 从内存跟踪中移除
                local idx = table.find(Library.MemoryTrack.Connections, Connection)
                if idx then
                    table.remove(Library.MemoryTrack.Connections, idx)
                end
            end
        end
...
function Library:OnUnload(Callback)
    ValidateType(Callback, "function", "Callback")
...
function Library:Unload()
    ...
    -- 清理内存跟踪的连接
    for Index = #Library.MemoryTrack.Connections, 1, -1 do
        local Connection = table.remove(Library.MemoryTrack.Connections, Index)
        if Connection and Connection.Connected then
            Connection:Disconnect()
        end
    end
    ...
    -- 清理渲染步连接
    for _, connection in pairs(Library.RenderConnections) do
    ...
    -- 尝试解绑渲染步
    pcall(function()
        RunService:UnbindFromRenderStep("Obsidian_Cursor")
    ...
    -- 清理内存跟踪
    Library.MemoryTrack.Instances = {}
    Library.MemoryTrack.Connections = {}
    ...
    if Library.DebugTools.Enabled then
        print("[Obsidian调试] 库已卸载，内存使用: " .. Library:GetMemoryUsage() .. " KB")
    end
end
...
function Library:SetIconModule(module: IconModule)
    ValidateType(module, "table", "module")
...
local BaseElement = {
    ...
    SetDisabled = function(self, disabled)
        ValidateType(disabled, "boolean", "disabled")
...
do
    ...
    function Funcs:AddKeyPicker(Idx, Info)
        ...
        if KeyPicker.Mode == "按下" then
            assert(ParentObj.Type == "Label", "模式为'按下'的KeyPicker只能应用于标签。")
        ...
        local KeybindsToggle = { Normal = KeyPicker.Mode ~= "开关" }
        do
        ...
        function KeyPicker:Update()
            ...
            if KeyPicker.Mode == "开关" and ParentObj.Type == "开关" and ParentObj.Disabled then
                KeybindsToggle:SetVisibility(false)
                return
            end

            local State = KeyPicker:GetState()
            local ShowToggle = Library.ShowToggleFrameInKeybinds and KeyPicker.Mode == "开关"
        ...
        Picker.MouseButton1Click:Connect(function()
            ...
            elseif Input.UserInputType == Enum.UserInputType.Keyboard then
                Key = Input.KeyCode == Enum.KeyCode.Escape and "无" or Input.KeyCode.Name
            end

            ActiveModifiers = if Input.KeyCode == Enum.KeyCode.Escape or Key == "未知" then {} else ActiveModifiers
        ...
        Library:GiveSignal(UserInputService.InputBegan:Connect(function(Input: InputObject)
            ...
            if KeyPicker.Mode == "开关" then
                if HoldingKey then
                    KeyPicker.Toggled = not KeyPicker.Toggled
                    KeyPicker:DoClick()
                end
            elseif KeyPicker.Mode == "按下" then
                if HoldingKey then
                    KeyPicker:DoClick()
                end
            end
...
do
    ...
    -- 公共切换开关创建函数（减少代码重复）
    local function CreateToggleBase(self, Idx, Info, isCheckbox)
        ...
        local Label = New("TextLabel", {
            BackgroundTransparency = 1,
            Position = UDim2.fromOffset(isCheckbox and 26 or 0, 0),
            Size = UDim2.new(1, isCheckbox and -26 or -40, 1, 0),
            Text = Toggle.Text,
            TextSize = 14,
            TextTransparency = 0.4,
            TextXAlignment = Enum.TextXAlignment.Left,
        ...
        if isCheckbox then
            ...
            function Toggle:Display()
                ...
                if Toggle.Disabled then
                    ...
                    ToggleElement.BackgroundColor3 = Library.Scheme.BackgroundColor
                    Library.Registry[ToggleElement].BackgroundColor3 = "BackgroundColor"
                ...
                ToggleElement.BackgroundColor3 = Library.Scheme.MainColor
                Library.Registry[ToggleElement].BackgroundColor3 = "MainColor"
            end
        else
            ToggleElement = New("Frame", {
                AnchorPoint = Vector2.new(1, 0),
            ...
            function Toggle:Display()
                ...
                ToggleElement.BackgroundTransparency = Toggle.Disabled and 0.75 or 0
                SwitchStroke.Transparency = Toggle.Disabled and 0.75 or 0

                ToggleElement.BackgroundColor3 = Toggle.Value and Library.Scheme.AccentColor or Library.Scheme.MainColor
                SwitchStroke.Color = Toggle.Value and Library.Scheme.AccentColor or Library.Scheme.OutlineColor

                Library.Registry[ToggleElement].BackgroundColor3 = Toggle.Value and "AccentColor" or "MainColor"
        ...
        function Toggle:SetValue(Value)
            ...
            for _, Addon in Toggle.Addons do
                if Addon.Type == "按键" and Addon.SyncToggleState then
    ...
    function Funcs:AddCheckbox(Idx, Info)
        return CreateToggleBase(self, Idx, Info, true)
    end
    ...
    function Funcs:AddToggle(Idx, Info)
        ...
        return CreateToggleBase(self, Idx, Info, false)
    end
    ...
    function Funcs:AddViewport(Idx, Info)
        ...
        if Info.Clone and typeof(Info.Object) == "Instance" then
            if ViewportObject.Archivable then
    ...
    function Funcs:AddImage(Idx, Info)
        ...
        function Image:SetColor(Color: Color3)
            assert(typeof(Color) == "Color3", "颜色必须是Color3值。")
        ...
        function Image:SetRectOffset(RectOffset: Vector2)
            assert(typeof(RectOffset) == "Vector2", "RectOffset必须是Vector2值。")
        ...
        function Image:SetRectSize(RectSize: Vector2)
            assert(typeof(RectSize) == "Vector2", "RectSize必须是Vector2值。")
        ...
        function Image:SetTransparency(Transparency: number)
            ...
            assert(Transparency >= 0 and Transparency <= 1, "透明度必须在0到1之间。")
    ...
    function Funcs:AddVideo(Idx, Info)
        ...
        function Video:SetVideo(NewVideo: string)
            assert(typeof(NewVideo) == "string", "视频必须是字符串。")
        ...
        function Video:SetLooped(Looped: boolean)
            assert(typeof(Looped) == "boolean", "循环必须是布尔值。")
        ...
        function Video:SetVolume(Volume: number)
            assert(typeof(Volume) == "number", "音量必须是0到10之间的数字。")
        ...
        function Video:SetPlaying(Playing: boolean)
            assert(typeof(Playing) == "boolean", "播放必须是布尔值。")
    ...
    function Funcs:AddDependencyBox()
        ...
        function Depbox:Update(CancelSearch)
            for _, Dependency in Depbox.Dependencies do
                ...
                if Element.Type == "开关" and Element.Value ~= Value then
                    DepboxContainer.Visible = false
                    Depbox.Visible = false
                    return
                elseif Element.Type == "下拉" then
        ...
        function Depbox:SetupDependencies(Dependencies)
            for _, Dependency in Dependencies do
                assert(typeof(Dependency) == "table", "依赖项必须是表格。")
                assert(Dependency[1] ~= nil, "依赖项缺少元素。")
                assert(Dependency[2] ~= nil, "依赖项缺少期望值。")
            end
...
function Library:SetFont(FontFace)
    ...
    ValidateType(FontFace, "userdata", "FontFace")
...
function Library:Notify(...)
    ...
    if typeof(Data.Time) == "Instance" then
        ...
        Library:GiveSignal(DeleteConnection)
    end
...
function Library:CreateWindow(WindowInfo)
    ValidateType(WindowInfo, "table", "WindowInfo")
    ...
    do
        Library.KeybindFrame, Library.KeybindContainer = Library:AddDraggableMenu("按键绑定")
    ...
    function Window:SetCompact(State)
        ValidateType(State, "boolean", "State")
    ...
    function Window:SetSidebarWidth(Width)
        ValidateType(Width, "number", "Width")
    ...
    function Window:ShowTabInfo(Name, Description)
        ValidateType(Name, "string", "Name", true)
        ValidateType(Description, "string", "Description", true)
    ...
    function Library:Toggle(Value: boolean?)
        ...
        if Library.Toggled and not Library.IsMobile then
            ...
            -- 保存取消函数
            Library.RenderConnections["Cursor"] = function()
                pcall(function()
                    RunService:UnbindFromRenderStep("Obsidian_Cursor")
            ...
            RunService:BindToRenderStep("Obsidian_Cursor", Enum.RenderPriority.Last.Value, function()
                ...
                if not (Library.Toggled and ScreenGui and ScreenGui.Parent) then
                    ...
                    if Library.RenderConnections["Cursor"] then
                        pcall(Library.RenderConnections["Cursor"])
                        Library.RenderConnections["Cursor"] = nil
                    end
                end
            end)
        elseif not Library.Toggled then
            ...
            for _, Option in Library.Options do
                if Option.Type == "颜色" then
                    Option.ColorMenu:Close()
                    Option.ContextMenu:Close()
                elseif Option.Type == "下拉" or Option.Type == "按键" then
                    Option.Menu:Close()
                end
            end
        ...
        -- 定期内存清理
        if Library.Toggled and tick() - Library.MemoryTrack.LastCleanup > 60 then
            Library:CleanupUnusedMemory()
        end
    end
    ...
    Library:GiveSignal(UserInputService.InputBegan:Connect(function(Input: InputObject)
        ...
        if
            (
                ...
                and Library.ToggleKeybind.Type == "按键"
...
local function OnPlayerChange()
    ...
    for _, Dropdown in Options do
        if Dropdown.Type == "下拉" and Dropdown.SpecialType == "Player" then
...
local function OnTeamChange()
    ...
    for _, Dropdown in Options do
        if Dropdown.Type == "下拉" and Dropdown.SpecialType == "Team" then
...
function Library:SetSafeMode(enabled)
    ValidateType(enabled, "boolean", "enabled")
    self.Config.SafeMode = enabled
    
    if enabled then
        print("[Obsidian] 安全模式已启用，外部资源加载被禁用")
    else
        print("[Obsidian] 安全模式已禁用")
    end
end

-- 添加性能监控切换
function Library:SetPerformanceLogging(enabled)
    ValidateType(enabled, "boolean", "enabled")
    self.Config.EnablePerformanceLogging = enabled
    
    if enabled then
        print("[Obsidian] 性能日志记录已启用")
    else
        print("[Obsidian] 性能日志记录已禁用")
    end
end

-- 添加内存快照功能
function Library:PrintMemoryInfo()
    local memory = self:GetMemoryUsage()
    local elements = self:GetElementCount()
    local instances = #self.MemoryTrack.Instances
    local connections = #self.MemoryTrack.Connections
    
    print(string.format("[Obsidian内存信息] 内存: %.2f KB, 元素: %d, 实例: %d, 连接: %d", 
          memory, elements, instances, connections))
